/**
 *  @file   ugprint.c
 *  @brief  print function for SD upgrading
 *  $Id: ugprint.c,v 1.6 2014/03/07 03:31:53 lym Exp $
 *  $Author: lym $
 *  $Revision: 1.6 $
 *
 *  Copyright (c) 2014 Terawins Inc. All rights reserved.
 * 
 *  @date   2014/01/04  lym New file.
 *
 */
...
__text__
int  __ug_doprnt(char *obuf, int bsize, const char *fmt0, va_list argp)
{
...
int ugprintf16(const char *fmt, ...)
{
    ...
    //assert(get_sp()    > (unsigned int)(&__STACK_BARRIER));

    va_start(ap, fmt);
    size = __ug_doprnt(ubuf, PBUF_SIZE, fmt, ap);
    va_end(ap);
